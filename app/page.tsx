...
import { ScrollReveal, ScrollWordReveal } from "./components/scroll-reveal";
...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    const scrollToSection = (direction: "down" | "up") => {
      const currentSections = mainElement.querySelectorAll("section");
      if (!currentSections.length) return;

      let targetIndex = activeSectionRef.current;
      if (direction === "down" && targetIndex < currentSections.length - 1) {
        targetIndex += 1;
      ...
      const targetSection = currentSections[targetIndex] as HTMLElement;
  ...
  return (
    <div className="relative h-screen overflow-hidden bg-background text-foreground flex flex-col selection:bg-indigo-500/30 selection:text-white">
      <GridBackground />
      ...
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-white/5 bg-background/60 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <Link href="/" className="flex items-center gap-2 group">
            <div className="h-3 w-3 rounded-full bg-indigo-500 group-hover:scale-125 transition-transform" />
          ...
          <nav className="hidden md:flex items-center gap-8 font-mono text-xs uppercase tracking-widest text-muted-foreground">
            ...
            <Link href="/media" className="flex items-center gap-1.5 hover:text-white transition-colors">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="h-10 px-5 rounded-full bg-indigo-500 font-mono text-xs font-bold uppercase tracking-widest text-white hover:bg-indigo-600 transition-all active:scale-95 shadow-[0_0_15px_rgba(99,102,241,0.25)]"
            >
              Launch App
            </button>
          </div>
      ...
      {/* HERO SECTION + SOCIAL PROOF (Viewport 1) */}
      <main ref={mainRef} className="flex-1 h-screen overflow-y-auto scroll-smooth w-full">
        <section className="relative min-h-screen snap-start flex flex-col justify-between pt-24 pb-8 w-full">
          <div className="max-w-6xl mx-auto px-6 w-full flex-1 flex flex-col justify-between">
            <div className="flex-1 flex items-center py-12 md:py-16">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center w-full">
                {/* Left Hero */}
                <div className="lg:col-span-7 space-y-8">
                  <ScrollReveal variant="fade" delay={0.1}>
                    <span className="inline-flex items-center gap-1.5 font-mono text-xs bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 px-3 py-1 rounded-full uppercase tracking-widest font-bold">
                  ...
                  <ScrollReveal variant="slide-up" delay={0.4}>
                    <div className="flex flex-wrap gap-4 pt-2">
                      <button
                        onClick={handleLaunchAppClick}
                        className="h-12 px-8 rounded-full bg-indigo-500 font-mono text-xs font-bold uppercase tracking-widest text-white hover:bg-indigo-600 transition-all hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(99,102,241,0.35)]"
                      >
                        Launch App
                      </button>
                      <Link
                        ...
                        className="h-12 px-8 rounded-full border border-white/10 bg-white/5 font-mono text-xs font-bold uppercase tracking-widest text-white hover:bg-white/10 hover:border-white/20 transition-all flex items-center justify-center gap-1.5"
                      >
                ...
                <div className="lg:col-span-5 relative flex items-center justify-center">
                  <ScrollReveal variant="scale-up" delay={0.4} className="w-full max-w-sm">
                    <div className="absolute inset-0 bg-indigo-500/10 rounded-full blur-[100px] pointer-events-none animate-pulse" />
                    <div className="relative w-full min-h-[380px] rounded-2xl border border-white/5 bg-card/20 p-8 backdrop-blur-xl flex flex-col justify-between shadow-2xl overflow-hidden group">
                      ...
                      <div className="flex items-center justify-between border-b border-white/5 pb-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-mono text-xs font-bold text-white uppercase tracking-wider">{"// ACTIVE_NODE_PREVIEW"}</span>
                        </div>
                        <span className="h-2.5 w-2.5 rounded-full bg-emerald-500 animate-pulse" />
                      </div>
                      <div className="py-8 space-y-4">
                        <div className="space-y-2">
                          <span className="text-xs font-mono text-muted-foreground uppercase">{"// CAMPAIGN_NAME"}</span>
                          <p className="text-2xl font-mono font-black text-white">Lagos_Geoclan_Drop_01</p>
                        </div>
                        <div className="grid grid-cols-2 gap-4 font-mono text-xs">
                          <div>
                            <span className="text-muted-foreground block mb-0.5">{"// REWARD"}</span>
                            <span className="text-emerald-400 font-bold text-sm">1.50 SOL</span>
                          </div>
                          <div>
                            <span className="text-muted-foreground block mb-0.5">{"// RADIUS"}</span>
                            <span className="text-white font-bold text-sm">150 Meters</span>
                          </div>
                        </div>
                      </div>
                      <button
                        ...
                        className="w-full h-12 rounded-lg border border-white/5 bg-white/5 hover:bg-indigo-500 hover:text-white transition-all flex items-center justify-center gap-2 font-mono text-xs font-bold uppercase tracking-widest text-muted-foreground"
                      >
            ...
            <ScrollReveal variant="fade" delay={0.6} threshold={0.01}>
              <div className="border-t border-white/5 pt-8 w-full">
                <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                  <span className="font-mono text-xs uppercase tracking-widest text-muted-foreground font-bold">{"// Built_With_The_Best"}</span>
                  <div className="flex flex-wrap justify-center items-center gap-10 opacity-50 grayscale hover:grayscale-0 transition-all duration-300">
                    <span className="font-mono text-sm md:text-base font-black text-white tracking-widest hover:text-indigo-400 transition-colors">SOLANA</span>
                    <span className="font-mono text-sm md:text-base font-black text-white tracking-widest hover:text-indigo-400 transition-colors">LI.FI BRIDGE</span>
                    <span className="font-mono text-sm md:text-base font-black text-white tracking-widest hover:text-indigo-400 transition-colors">LEAFLET MAPS</span>
                    <span className="font-mono text-sm md:text-base font-black text-white tracking-widest hover:text-indigo-400 transition-colors">CODAMA API</span>
                  </div>
                </div>
              </div>
            </ScrollReveal>
        ...
        {/* PROBLEM STATEMENT (Viewport 2) */}
        <section className="min-h-screen snap-start flex flex-col justify-center w-full border-b border-white/5 py-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
              <ScrollReveal variant="slide-right" delay={0.1}>
                <div className="space-y-4">
                  <span className="font-mono text-xs text-indigo-400 uppercase tracking-widest block font-bold">{"// THE_INJUSTICE"}</span>
                  <h2 className="text-3xl md:text-4xl lg:text-5xl font-mono font-black uppercase tracking-tight text-white break-all">
        ...
        <section className="min-h-screen snap-start flex flex-col justify-center w-full border-b border-white/5 py-24">
          <div className="max-w-6xl mx-auto px-6 w-full flex-1 flex flex-col justify-center">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
              <ScrollReveal variant="slide-right" delay={0.1}>
                <div className="space-y-6">
                  <span className="font-mono text-xs text-indigo-400 uppercase tracking-widest block font-bold">{"// THE_SOLUTION"}</span>
                  <h2 className="text-3xl md:text-4xl lg:text-5xl font-mono font-black uppercase tracking-tight text-white leading-none break-all">
              ...
              <ScrollReveal variant="scale-up" delay={0.3}>
                <div className="rounded-2xl border border-white/5 bg-card/20 p-8 backdrop-blur-xl space-y-6 font-mono text-xs md:text-sm shadow-2xl">
                  <div className="space-y-4">
        ...
        {/* BENEFITS (Viewport 4) */}
        <section className="min-h-screen snap-start flex flex-col justify-center w-full border-b border-white/5 py-24">
          <div className="max-w-6xl mx-auto px-6 w-full flex-1 flex flex-col justify-center space-y-12">
            <ScrollReveal variant="slide-up" delay={0.1}>
              <div className="space-y-4 text-center">
                <span className="font-mono text-xs text-indigo-400 uppercase tracking-widest block font-bold">{"// CORE_BENEFITS"}</span>
                <h2 className="text-3xl md:text-4xl lg:text-5xl font-mono font-black uppercase tracking-tight text-white break-all">
                  Why_GeoDrop_Works
                </h2>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <ScrollReveal variant="slide-up" delay={0.2}>
                <div className="rounded-xl border border-white/5 bg-card/30 p-6 md:p-8 backdrop-blur-xl space-y-4 hover:border-indigo-500/30 transition-all duration-300">
                  <div className="h-10 w-10 rounded-lg bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-indigo-500">
                    <Shield className="h-5 w-5" />
                  </div>
                  <h3 className="font-mono text-base font-bold uppercase text-white tracking-wide">Zero_Bot_Claims</h3>
              ...
              <ScrollReveal variant="slide-up" delay={0.3}>
                <div className="rounded-xl border border-white/5 bg-card/30 p-6 md:p-8 backdrop-blur-xl space-y-4 hover:border-indigo-500/30 transition-all duration-300">
                  ...
                  <h3 className="font-mono text-base font-bold uppercase text-white tracking-wide">Boots_on_the_Ground</h3>
                  <p className="text-sm text-foreground/50 leading-relaxed">
              ...
              <ScrollReveal variant="slide-up" delay={0.4}>
                <div className="rounded-xl border border-white/5 bg-card/30 p-6 md:p-8 backdrop-blur-xl space-y-4 hover:border-indigo-500/30 transition-all duration-300">
                  ...
                  <h3 className="font-mono text-base font-bold uppercase text-white tracking-wide">Cross-Chain_Funded</h3>
                  <p className="text-sm text-foreground/50 leading-relaxed">
        ...
        {/* HOW IT WORKS (Viewport 5) */}
        <section className="min-h-screen snap-start flex flex-col justify-center w-full border-b border-white/5 py-24">
          <div className="max-w-6xl mx-auto px-6 w-full flex-1 flex flex-col justify-center space-y-12">
            <ScrollReveal variant="slide-up" delay={0.1}>
              <div className="space-y-4 text-center">
                <span className="font-mono text-xs text-indigo-400 uppercase tracking-widest block font-bold">{"// PROCESS_WORKFLOW"}</span>
                <h2 className="text-3xl md:text-4xl lg:text-5xl font-mono font-black uppercase tracking-tight text-white break-all">
                  Three_Simple_Steps
                </h2>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <ScrollReveal variant="slide-right" delay={0.2}>
                <div className="space-y-4 relative p-8 bg-card/10 border border-white/5 rounded-xl hover:border-indigo-500/20 transition-all">
                  <span className="text-6xl font-mono font-black text-indigo-500/30">01</span>
                  <h3 className="font-mono text-base font-bold uppercase text-white">Deploy_Campaign</h3>
                  <p className="text-sm text-foreground/50 leading-relaxed">
              ...
              <ScrollReveal variant="slide-up" delay={0.3}>
                <div className="space-y-4 relative p-8 bg-card/10 border border-white/5 rounded-xl hover:border-indigo-500/20 transition-all">
                  <span className="text-6xl font-mono font-black text-indigo-500/30">02</span>
                  <h3 className="font-mono text-base font-bold uppercase text-white">Deposit_Escrow</h3>
                  <p className="text-sm text-foreground/50 leading-relaxed">
              ...
              <ScrollReveal variant="slide-left" delay={0.4}>
                <div className="space-y-4 relative p-8 bg-card/10 border border-white/5 rounded-xl hover:border-indigo-500/20 transition-all">
                  <span className="text-6xl font-mono font-black text-indigo-500/30">03</span>
                  <h3 className="font-mono text-base font-bold uppercase text-white">IRL_Claim</h3>
                  <p className="text-sm text-foreground/50 leading-relaxed">
        ...
        {/* BELIEFS MANIFESTO (Viewport 6) */}
        <section className="min-h-screen snap-start flex flex-col justify-center w-full border-b border-white/5 py-24">
          <div className="max-w-4xl mx-auto px-6 w-full flex-1 flex flex-col justify-center text-center space-y-6">
            <ScrollReveal variant="fade" delay={0.1}>
              <span className="font-mono text-xs text-indigo-400 uppercase tracking-widest block font-bold">{"// MANIFESTO"}</span>
            </ScrollReveal>
            <ScrollWordReveal
              text="&quot;We believe crypto should touch the ground. We refuse to accept that token distribution means airdropping to anonymous wallets and praying they're real people. GeoDrop is built on the conviction that the best communities are built face-to-face, not screen-to-screen.&quot;"
              className="text-2xl sm:text-3xl md:text-4xl font-mono uppercase font-black leading-snug"
            />
          </div>
        </section>

        {/* FINAL CTA & FOOTER (Viewport 7) */}
        <section className="min-h-screen snap-end flex flex-col justify-between pt-24 pb-8 w-full">
          <div className="max-w-6xl mx-auto px-6 w-full flex-1 flex flex-col justify-between">
            {/* FINAL CTA SECTION */}
            <section className="flex-1 flex items-center justify-center py-8">
              <ScrollReveal variant="scale-up" duration={0.8} className="w-full">
                <div className="w-full rounded-2xl border border-white/5 bg-indigo-500/5 p-12 backdrop-blur-xl text-center space-y-8 shadow-[0_0_50px_rgba(99,102,241,0.05)] relative overflow-hidden">
                  <div className="absolute inset-0 bg-gradient-to-b from-indigo-500/[0.02] to-transparent pointer-events-none" />
                  <div className="space-y-3">
                    <span className="font-mono text-xs text-indigo-400 uppercase tracking-widest block font-bold">{"// EXPLORE_NOW"}</span>
                    <h2 className="text-3xl md:text-4xl lg:text-5xl font-mono font-black uppercase tracking-tight text-white">
                      Ready_To_Claim_Your_First_Drop?
                    </h2>
                    <p className="text-sm md:text-base text-foreground/50 max-w-lg mx-auto leading-relaxed">
                  ...
                  <div className="flex flex-col sm:flex-row justify-center items-center gap-4 pt-2">
                    ...
                    <Link
                      ...
                      className="h-12 px-6 rounded-full border border-white/10 bg-white/5 font-mono text-xs font-bold uppercase tracking-widest text-white hover:bg-white/10 active:scale-95 flex items-center justify-center"
                    >
                      Join Waitlist
                    </Link>
                    <Link
            ...
            {/* FOOTER */}
            <footer className="border-t border-white/5 py-8 mt-12 w-full">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                <div className="space-y-4">
                  <Link href="/" className="flex items-center gap-2">
                    <div className="h-3 w-3 rounded-full bg-indigo-500" />
                    <span className="font-mono text-sm font-bold uppercase tracking-widest text-white">GeoDrop</span>
                  </Link>
              ...
              <div className="pt-8 mt-8 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between text-xs font-mono uppercase tracking-widest text-muted-foreground gap-4">
