...
import { lamports as sol, type Account, address as addressHelper, type ReadonlyUint8Array } from "@solana/kit";
  ...
  useEffect(() => {
    async function fetchCampaigns() {
      ...
      try {
        ...
        addLog(`[GeoDrop] RPC returned ${programAccounts.length} program accounts.`);
        ...
        addLog(`[GeoDrop] Successfully synced ${decodedDrops.length} campaigns.`);
        setMyDrops(decodedDrops);
      } catch (e) {
        console.error("[GeoDrop] Dashboard sync failed:", e);
        addLog(`[GeoDrop] Error: ${e instanceof Error ? e.message : 'Unknown RPC error'}`);
        setMyDrops([]);
      } finally {
      ...
      <div className="relative z-10">
        <header className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <Link href="/" className="font-mono text-xs font-bold uppercase tracking-widest text-white">
            ...
            <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
              <div className="flex flex-col gap-4">
                <h1 className="font-black tracking-tight text-foreground">
                  <span className="block text-6xl md:text-7xl text-white">Geo</span>
              ...
              <section className="space-y-6">
                <div className="flex items-center justify-between border-b border-white/5 pb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="font-mono text-sm font-bold uppercase tracking-widest text-white">My_Active_Campaigns</h2>
                  </div>
                  {myDrops.length > 0 && (
                    <span className="font-mono text-[10px] text-muted-foreground uppercase">Total_Nodes: 0{myDrops.length}</span>
                  )}
                </div>
                ...
                {isLoadingDrops ? (
                  <div className="flex h-48 w-full flex-col items-center justify-center gap-4 rounded-2xl border border-dashed border-white/5 bg-white/[0.02]">
                    ...
                    <p className="font-mono text-[10px] text-muted-foreground uppercase tracking-widest">SYNCHRONIZING_WITH_BLOCKCHAIN...</p>
                  </div>
                ) : myDrops.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {myDrops.map((drop) => <CampaignCard key={drop.address} drop={drop} />)}
                  </div>
                ) : (
                  <div className="flex h-48 w-full flex-col items-center justify-center gap-6 rounded-2xl border border-dashed border-white/5 bg-white/[0.02] text-center px-6">
                    <p className="max-w-xs text-xs text-muted-foreground leading-relaxed">
                      No active drops found for this sponsor address. Ensure you are connected to the correct cluster and wallet.
                    </p>
                    <Link href="/campaign/create" className="inline-flex items-center gap-2 font-mono text-[10px] font-bold uppercase tracking-widest text-indigo-400 hover:text-indigo-300 transition-colors">
                      [+] DEPLOY_NEW_CAMPAIGN
                    </Link>
                  </div>
                )}
                ...
                {status === "connected" && debugLog.length > 0 && (
                  <div className="mt-8 rounded-xl border border-white/5 bg-black/40 p-4">
                    ...
                    <div className="space-y-1">
                      {debugLog.map((log, i) => (
                        <p key={i} className="font-mono text-[9px] text-muted-foreground break-all">{log}</p>
                      ))}
                    </div>
                  </div>
                )}
                ...
                <section className="relative w-full overflow-hidden rounded-2xl border border-white/5 bg-card/50 p-6 backdrop-blur-xl">
                  <div className="relative flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold uppercase tracking-widest font-mono text-white">Vault_Status</span>
                    </div>
                    {cluster !== "mainnet" && (
                      <button onClick={handleAirdrop} className="cursor-pointer font-mono text-[8px] uppercase tracking-widest text-indigo-400 underline underline-offset-4 hover:text-indigo-300">[ Faucet ]</button>
                    )}
                  </div>
                  <div className="mt-6 space-y-1">
                    <p className="font-mono text-xs text-muted-foreground uppercase">Liquid_SOL</p>
                  ...
                  <button onClick={handleCopy} className="mt-6 flex w-full cursor-pointer items-center justify-between rounded-lg border border-white/5 bg-white/5 px-3 py-2 transition hover:bg-white/10">
                    <span className="font-mono text-[10px] text-muted-foreground">{ellipsify(address, 6)}</span>
      ...
      {showDemo && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
          <div className="relative w-full max-w-4xl p-4 animate-in zoom-in-95 duration-300">
            <button onClick={() => setShowDemo(false)} className="absolute -top-12 right-4 flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-white hover:bg-white/20 transition-colors">
              <X className="h-6 w-6" />
            </button>
            <div className="aspect-video w-full overflow-hidden rounded-2xl border border-white/10 bg-black shadow-2xl">
              <iframe width="100%" height="100%" src="https://www.youtube.com/embed/aog9_mYFT28?si=Ch4qEukIvS_vX3C6&autoplay=1" title="YouTube video player" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen />
            </div>
          </div>
        </div>
      )}
