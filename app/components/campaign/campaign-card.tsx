...
import { MapPin, Target, Users, Award, ExternalLink, Tag } from "lucide-react";
...
export function CampaignCard({ drop }: CampaignCardProps) {
  ...
  const progressPercent = (Number(data.currentClaims) / Number(data.maxClaims)) * 100;
    ...
    <div className="group relative flex flex-col gap-6 rounded-2xl border border-white/5 bg-white/5 p-6 transition-all hover:border-indigo-500/30 hover:bg-indigo-500/5 shadow-2xl">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-1 min-w-0 flex-1">
           <div className="flex items-center gap-2">
              <div className={`h-1.5 w-1.5 rounded-full ${remainingClaims > 0 ? "bg-emerald-500 animate-pulse" : "bg-red-500"}`} />
              <span className={`font-mono text-[8px] font-bold uppercase tracking-widest ${remainingClaims > 0 ? "text-emerald-400" : "text-red-400"}`}>
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-end">
          <div>
            <p className="text-[9px] font-mono text-muted-foreground uppercase tracking-widest mb-1">Reward_Per_Winner</p>
            <div className="flex items-baseline gap-1.5">
              ...
              <span className="text-xs font-bold text-indigo-400 uppercase">Sol</span>
            </div>
          </div>
          <div className="text-right">
            <p className="text-[9px] font-mono text-muted-foreground uppercase tracking-widest mb-1">Claims</p>
            <p className="font-mono text-xl font-bold text-white leading-none">
              {data.currentClaims.toString()}<span className="text-muted-foreground/30 mx-1">/</span>{data.maxClaims.toString()}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 border-t border-white/5 pt-4">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-1.5 text-muted-foreground">
              ...
              <span className="text-[9px] font-mono uppercase tracking-widest">Radius</span>
            </div>
            <p className="font-mono text-[11px] text-white uppercase tracking-tighter">
              {data.radius.toString()}<span className="text-[8px] ml-0.5 opacity-50">Meters</span>
            </p>
          </div>
        </div>
      </div>
